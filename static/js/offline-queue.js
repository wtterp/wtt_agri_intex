(() => {
  const KEY = 'wtt_exhibition_lead_offline_queue_v1';
  ...
  function add(payload) {
    ...
    if (!items.some(item => item.client_submission_id === payload.client_submission_id)) items.push(payload);
  ...
  async function flush() {
    ...
    for (let i=0;i<items.length;i++) {
      ...
      try {
        ...
        if (response.ok) sent += 1; else remaining.push(item);
      } catch (_) { remaining.push(item, ...items.slice(i+1)); break; }
    }
    save(remaining, sent > 0 || remaining.length !== items.length);
    return {sent, remaining:remaining.length};
  }
  window.WTT_QUEUE = {load, add, flush};
})();
