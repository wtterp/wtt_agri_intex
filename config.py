...
class Config:
    ...
    # VERCEL
    # ---------------------------------------------------------
    # Vercel sets VERCEL=1 automatically in its runtime.
    ...
    # Only /tmp is writable inside a serverless function.
    #
    # IMPORTANT:
    # /tmp is temporary/ephemeral. It is suitable for making the
    # current SQLite-based app run on Vercel, but it is NOT a
    # permanent production database.
    if IS_VERCEL:
        DATABASE_PATH = "/tmp/wtt_agri_intex.db"
    else:
        _db_value = os.getenv(
            "DATABASE_PATH",
            "instance/wtt_agri_intex.db",
        )
        DATABASE_PATH = (
            str((BASE_DIR / _db_value).resolve())
            if not os.path.isabs(_db_value)
            else _db_value
        )

    # ---------------------------------------------------------
    # OPENAI - VISITING CARD SCANNER
    # ---------------------------------------------------------
    # Keep this key only on the server / Vercel Environment Variables.
    # Never expose it in JavaScript or HTML.
    OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "").strip()
    OPENAI_VISION_MODEL = (
        os.getenv("OPENAI_VISION_MODEL", "gpt-5.6-luna").strip()
        or "gpt-5.6-luna"
    )
